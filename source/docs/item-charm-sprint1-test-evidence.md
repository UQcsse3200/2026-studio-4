...
2. `ItemFactory.createDrop()` returns a new, unregistered Strength Charm entity.
...
`ItemFactory.createDrop()` owns the item entity's component setup. It returns an entity containing:
...
2. Trigger an enemy death in a room that requests `ItemFactory.createDrop()`.
