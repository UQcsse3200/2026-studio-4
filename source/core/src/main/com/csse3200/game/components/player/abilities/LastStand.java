 ...
 * last stand effect on them, amplifying effective strength, movement speed and attack speed for as
 * long as it runs.
 *
 * <p>The ability only decides which hit applies the effect. How much the effect amplifies by, and
 * everything that reads it, lives on the effect and the status effects controller.
 */
public final class LastStand extends TimedPlayerAbility {
  ...
  public LastStand(GameTime time) {
    super(NAME, COOLDOWN_MS, false, () -> StatusEffectsFactory.createLastStand(time, DURATION_MS));
  }
