...
public class InventoryDisplay extends UIComponent {
  ...
  /**
   * Registers an inventory slot as a slot that can have an item dropped into
   *
   * @param slotBackground The inventory slot where an item in the inventory can be moved to
   */
  ...
  private void charmIconDraw(
    ...
    if (currentCharm.equals("Speed Charm")) {
      Image speedCharmIcon = new Image(skin, "button-c"); // change to icon when given
  ...
  private void consumableIconDraw(
    ...
    if (currentCharm.equals("Strength Charm")) {
      ...
      if (quantity >= 1) {
        ...
        textOverlayTable.add(quantityLabel).padBottom(2).padRight(4);
        slotStack.add(textOverlayTable);
        // register the stack of the consumable drawing as a draggable
        // and droppable visual component
